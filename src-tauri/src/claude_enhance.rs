...
function cpuTick(){if(!window.__claudePlusEnhanceTokenUsageV1){cpuRender();return}if(!cpu.polling){cpu.polling=!0;cpuPoll();setInterval(cpuPoll,1200)}}
...
D();
    ...
    mod tests {
        ...
        fn token_usage_polls_local_proxy_usage_and_renders_badge() {
            ...
            assert!(INJECT_SCRIPT.contains("function cpuTick(){if(!window.__claudePlusEnhanceTokenUsageV1){cpuRender();return}if(!cpu.polling)"));
