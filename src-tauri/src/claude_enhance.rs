...
function summary(e,t){let r=(t||"").replace(/\s+/g," ").trim();if(!r||r==="未提供描述")return`处理「${e}」相关工作流。`;r=r.replace(/^Use when /i,"适用于").replace(/^Use this when /i,"适用于").replace(/^Use this skill when /i,"适用于").replace(/^Use to /i,"用于").replace(/^Creates? /i,"创建").replace(/^Builds? /i,"构建").replace(/^Inspect /i,"检查").replace(/^Analyze /i,"分析").replace(/^Guide /i,"引导").replace(/^Manage /i,"管理").replace(/^Install /i,"安装");if(!/[。.!?]$/.test(r))r+="。";return Array.from(r).slice(0,96).join("")}
...
function C(e,n){const t=e.filter(e=>e.scope===n),r=n==="global"?"全局 skills":"项目 skills";return'<section class="cps-section"><div class="cps-section-title">'+r+'</div><div class="cps-container">'+(t.length?t.map(e=>'<article class="cps-card" data-id="'+z(e.id)+'"><div class="cps-main"><div class="cps-name">'+z(e.name)+'</div><div class="cps-brief">'+z(e.summary_zh||"未提供功能概述。")+'</div><div class="cps-file">'+z(e.skill_file||e.path)+'</div></div><div class="cps-actions"><button type="button" class="cps-danger" data-cps-trash>删除</button></div></article>').join(""):'<p class="cps-empty">暂无'+r+'。</p>')+"</div></section>"}
