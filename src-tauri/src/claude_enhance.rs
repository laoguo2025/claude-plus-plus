...
const cpu={last:null,lastId:0,polling:!1,pollBusy:!1,lastPollAt:0,pending:!1,seq:0,recent:[],debug:[]};
window.__claudePlusTokenUsage={last:null,recent:cpu.recent,debug:cpu.debug};
...
function cpuRemember(e,t){const n=cpuExtractUsages(e);if(!n.length)return!1;const r=n.reduce((e,n)=>({id:++cpu.seq,input:e.input+n.input,output:e.output+n.output,total:e.total+n.total,cached:e.cached+n.cached,cacheReadTokens:e.cacheReadTokens+n.cacheReadTokens,cacheCreationTokens:e.cacheCreationTokens+n.cacheCreationTokens,cachedReadTokens:e.cachedReadTokens+n.cachedReadTokens,cacheCreated:e.cacheCreated+n.cacheCreated,contextUsed:Math.max(e.contextUsed,n.contextUsed),contextLimit:Math.max(e.contextLimit,n.contextLimit),elapsed:t||e.elapsed,updatedAt:Date.now()}),{id:0,input:0,output:0,total:0,cached:0,cacheReadTokens:0,cacheCreationTokens:0,cachedReadTokens:0,cacheCreated:0,contextUsed:0,contextLimit:0,elapsed:0,updatedAt:0});cpu.lastId=r.id;cpu.last=r;cpu.pending=!1;cpu.recent.unshift(r);cpu.recent=cpu.recent.slice(0,20);window.__claudePlusTokenUsage={last:r,recent:cpu.recent,debug:cpu.debug};cpuRender();return!0}
...
function cpuStart(e){if(cpuSend(e)){cpu.pending=!0;cpuClear();cpuPoll(!0)}}
...
D();
    ...
    mod tests {
        ...
        fn token_usage_polls_local_proxy_usage_and_renders_badge() {
            ...
            assert!(INJECT_SCRIPT.contains("if(cpuBusy()){if(!cpu.pending){cpu.pending=!0;cpuClear()}cpuPoll(!0);return}"));
            assert!(INJECT_SCRIPT
