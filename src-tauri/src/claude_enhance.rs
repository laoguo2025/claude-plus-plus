...
function aa(e){const n=E(e.textContent);return/(打开于|Open|固定|Pin|标记为未读|Unread|重命名|Rename|分叉|Fork|存档|Archive|删除|Delete)/i.test(n)&&!/(导出 Markdown|Export Markdown)/i.test(n)}
function ab(e){const n=document.createElement("button");n.type="button";n.className=e?.className||"claude-plus-markdown-menu-item";n.classList.add("claude-plus-markdown-menu-item");n.setAttribute("role",e?.getAttribute("role")||"menuitem");n.setAttribute("tabindex",e?.getAttribute("tabindex")||"0");n.dataset.claudePlusMarkdownMenuItem="1";if(e){n.innerHTML=e.innerHTML;const t=Array.from(n.querySelectorAll("*")).find(e=>E(e.textContent)&&!/[A-Z]$/.test(E(e.textContent)));t?t.textContent="导出 Markdown":n.textContent="导出 Markdown"}else n.textContent="导出 Markdown";n.querySelectorAll("[aria-hidden='true'],svg").forEach(e=>e.remove());n.addEventListener("click",e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation?.();Q()},!0);n.addEventListener("keydown",e=>{(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),Q())},!0);return n}
function P(){document.querySelectorAll(".claude-plus-markdown-export").forEach(e=>e.remove());if(!window.__claudePlusEnhanceMarkdownExportV1)return;O();document.querySelectorAll('[role="menu"],[data-radix-popper-content-wrapper],[data-radix-menu-content],div[cmdk-list]').forEach(e=>{if(e.querySelector("[data-claude-plus-markdown-menu-item]"))return;const n=Array.from(e.querySelectorAll('button,[role="menuitem"],[cmdk-item],div')).filter(aa);if(!n.length)return;const t=n.find(e=>/存档|Archive/i.test(E(e.textContent)))||n.find(e=>/删除|Delete/i.test(E(e.textContent)))||n[n.length-1];const r=ab(t);t.parentElement?t.parentElement.insertBefore(r,t):e.appendChild(r)})}
...
D();
    ...
    mod tests {
        ...
        fn markdown_export_is_inserted_into_conversation_menu() {
            ...
            assert!(INJECT_SCRIPT.contains("insertBefore(r,t)"));
