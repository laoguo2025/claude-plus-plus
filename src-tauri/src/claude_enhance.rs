...
const cpu={last:null,lastId:0,polling:!1,pollBusy:!1,lastPollAt:0,pending:!1};
...
function cpuHtml(e){const t=e.input||0,n=Math.min(e.cached||0,t),r=e.contextUsed||e.total||0,a=e.contextLimit||0,s=a?cpuPct(r,a):"0%",l=t?cpuPct(n,t):"0%";return'<div class="cpu-line">总计 <strong>'+cpuF(e.total)+"</strong> · 输入 "+cpuF(e.input)+" · 输出 "+cpuF(e.output)+'</div><div class="cpu-line cpu-muted">缓存命中 '+cpuF(n)+" · 缓存命中率 "+l+" · 上下文 "+cpuF(r)+(a?"/"+cpuF(a):"")+" ("+s+") · 耗时 "+((e.elapsed||0)/1000).toFixed(1)+"s</div>"}
...
D();
    ...
    mod tests {
        ...
        fn token_usage_polls_local_proxy_usage_and_renders_badge() {
            ...
            assert!(INJECT_SCRIPT.contains("Math.min(e.cached||0,t)"));
