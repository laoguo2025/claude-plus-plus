...
function s(e){const n=new URL(e.path,location.origin);if(e.open==="window"){try{const e=window.open(n.toString(),"_blank");if(e)return}catch(t){}}const t=n.pathname+n.search+n.hash;try{history.pushState(null,"",t);window.dispatchEvent(new PopStateEvent("popstate",{state:history.state}));window.dispatchEvent(new Event("pushstate"));window.dispatchEvent(new Event("locationchange"))}catch(r){location.assign(n.toString())}}
