...
- Welcome page startup checks must stay lightweight and avoid blocking on full Claude Desktop resource scans, CC Switch mapping reads, or page enhancement bundle/asar reads. The welcome cards use `welcome_status`; route, localization, and enhancement details are loaded only when their pages are opened or when explicitly refreshed on that page.
