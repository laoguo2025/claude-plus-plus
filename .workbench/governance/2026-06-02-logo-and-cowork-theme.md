...
用户要求菜单中 `关于工具` 移到 `诊断日志` 下方，并参考 Codex++ 关于页重做内容；暂时没有的信息先留空，且不要 Discord 按钮。本次调整：

- 左侧菜单顺序改为 `CCS转接`、`一键汉化`、`页面增强`、`诊断日志`、`关于工具`。
- `关于工具` 页从旧的说明卡 + 状态卡重构为单页信息面板。
- 保留当前可用信息：版本、Claude++ 转接、当前路由、汉化状态。
- GitHub 仓库、Release 信息、许可证、维护者、反馈入口显示 `待补充`，等待推送 GitHub 后再补。
- 未添加 Discord 文案或按钮。
- 浏览器预览验证菜单顺序、关于页信息项和无 Discord 文案；仅执行 `npm run build`，未打包。
